...
interface AuthorCredentialsMail {
  email: string;
  password: string;
  name?: string | null;
}
...
export class AppMailService {
  ...
  async sendAuthorCredentials({
    email,
    password,
    name,
  }: AuthorCredentialsMail) {
    ...
    await this.mailerService.sendMail({
      to: email,
      subject: 'Your author account login credentials',
      text: [
        `Hello ${recipientName},`,
        '',
        'Your author account has been created.',
        '',
        `Username/Email: ${email}`,
        `Password: ${password}`,
        '',
      ...
      html: [
        `<p>Hello ${safeRecipientName},</p>`,
        '<p>Your author account has been created.</p>',
        '<p>',
        `<strong>Username/Email:</strong> ${safeEmail}<br />`,
        `<strong>Password:</strong> ${safePassword}`,
        '</p>',
